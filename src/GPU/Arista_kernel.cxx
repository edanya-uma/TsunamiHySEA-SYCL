...
double limitador(double p0m, double p0, double p1, double p1p, double S)
{
    ...
    c0 = abs(p1-p0);
    c1 = abs((p1p-p1)*(S<0.0) + (p0-p0m)*(S>0.0) + 0.5*(p1p-p0m)*(S==0.0));
...
double limitador_monotono(double p0m, double p0, double p1, double p1p, double S)
{
    double chi;
    double vs;
    double c0,c1;
...
void tratamientoSecoMojado(double h0, double h1, double eta0, double eta1, double H0, double H1,
...
void positividad(double h0, double h1, double delta_T, double area0, double area1, double *Fmenos)
{
    double dt0;
    double factor = 0.25;
    double alpha;
...
void positividad_H(double h0, double h1, double cosPhi0, double cosPhi1, double delta_T,
...
// ARISTAS VERTICALES
void procesarAristaVer_2s_1step(double h0, double h1, double q0x, double longitud, double area0,
     ...
     double hpos, double *Fmas2s, double *Fmenos2s, bool alfa_cero)
{
    ...
    if ((h0 > EPSILON) || (h1 > EPSILON)) {
        ...
        h0rg = sqrt2*h0 / sqrt(h0*h0*h0*h0 + maxheps*maxheps*maxheps*maxheps);
        u0n = h0rg*q0n;
        maxheps = max(h1,epsilon_h);
        h1rg = sqrt2*h1 / sqrt(h1*h1*h1*h1 + maxheps*maxheps*maxheps*maxheps);
        u1n = h1rg*q1n;
        q0n = u0n*h0;
        q1n = u1n*h1;
        *Fmenos2s = ilcos*q0n*normal;
        ...
        max_autovalor = max(abs(aut2), abs(aut1));
     ...
     TVec2 *Fmenos2, bool alfa_cero)
{
    ...
    if ((W0->x() > EPSILON) || (W1->x() > EPSILON)) {
        ...
        p0m = sqrt2*h0m*q0mt / sqrt(h0m*h0m*h0m*h0m + maxheps*maxheps*maxheps*maxheps);
        p0 = u0n = h0rg*q0t;
        p1 = u1n = h1rg*q1t;
        maxheps = max(h1p,epsilon_h);
        p1p = sqrt2*h1p*q1pt / sqrt(h1p*h1p*h1p*h1p + maxheps*maxheps*maxheps*maxheps);
...
void procesarAristaHor_2s_1step(double h0, double h1, double q0y, double longitud, double area0,
     double area1, double delta_T, double2 *d_acumulador_1, int pos_vol0, int pos_vol1, double cosPhi0,
...
void procesarAristasHorNivel0Paso1GPU( nd_item<2> idx,
     double2 *d_datosVolumenes_1, double2 *d_datosVolumenes_2,
...
void procesarAristasHorNivel0Paso2GPU( nd_item<2> idx,
     double2 *d_datosVolumenes_1, double2 *d_datosVolumenes_2,
     double2 *d_datosVolumenes_3, double *d_anchoVolumenes, int num_volx, int num_voly,
     ...
     double longitud, double2 *d_acumulador_2, int pos_vol0, int pos_vol1, double epsilon_h, double cosPhi0)
{
    ...
    maxheps = max(h0,epsilon_h);
    u0n = sqrt2*h0*W0_rot.y() / sqrt(h0*h0*h0*h0 + maxheps*maxheps*maxheps*maxheps);
    maxheps = max(h1,epsilon_h);
    u1n = sqrt2*h1*W1_rot.y() / sqrt(h1*h1*h1*h1 + maxheps*maxheps*maxheps*maxheps);
    sqrt_h0 = sqrt(h0);
    sqrt_h1 = sqrt(h1);
    u1ij_n = (sqrt_h0*u0n + sqrt_h1*u1n) / (sqrt_h0 + sqrt_h1 + EPSILON);

    max_autovalor = abs(u1ij_n)+sqrt(h1ij);
...
void procesarAristasHorDeltaTInicialNivel0GPU( nd_item<2> idx,
     double2 *d_datosVolumenesNivel0_1, double2 *d_datosVolumenesNivel0_2,
