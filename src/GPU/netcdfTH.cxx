...
namespace netcdfTH
{
	...
	void escribirDatosCroppingNC(int ncid, int crop_flag, double crop_value, double H)
	{
		...
		if (crop_flag == CROP_RELATIVE) {
			...
			check_err(iret);
			val_float = static_cast<float>(crop_value);
			...
			check_err(iret);
			val_float = static_cast<float>(crop_value*H);
	...
	void crearFicheroNC(double *lon_grid, double *lat_grid, double *lon, double *lat, char *nombre_bati, int okada_flag,
				...
				int crop_flag, double crop_value, double H, float *bati, char *version)
	{
		constexpr int maxCadena = 512;
		char cadena[maxCadena];
		char nombre_falla[maxCadena];
		...
		iret = nc_put_att_text(ncid, NC_GLOBAL, "problem_name", strlen(nombre_bati), nombre_bati);
		check_err(iret);
		val_float = static_cast<float>(tiempo_tot);
		...
		check_err(iret);
		if (es_periodica) {
			sprintf(cadena, "periodic");
			iret = nc_put_att_text(ncid, NC_GLOBAL, "left_border", 8, cadena);
			check_err(iret);
			iret = nc_put_att_text(ncid, NC_GLOBAL, "right_border", 8, cadena);
			check_err(iret);
		}
		else {
		...
		if (okada_flag == SEA_SURFACE_FROM_FILE) {
			...
			for (i=0; i<numFaults; i++) {
				snprintf(nombre_falla, maxCadena, "fault_%d", i+1);
				snprintf(cadena, maxCadena, "time: %.4f, lon: %.4f, lat: %.4f, depth: %.4f, length: %.4f, width: %.4f, strike: %.4f, dip: %.4f, rake: %.4f, slip: %.4f",
					defTime[i], LON_C[i], LAT_C[i], DEPTH_C[i], FAULT_L[i], FAULT_W[i], STRIKE[i], DIP[i], RAKE[i], SLIP[i]);
				iret = nc_put_att_text(ncid, NC_GLOBAL, nombre_falla, strlen(cadena), cadena);
				check_err(iret);
			}
				...
				double crop_value, double H, float *bati, char *version)
	{
		...
		ErrorEnNetCDF = false;
		lon = static_cast<double *>(malloc(num_volx*sizeof(double)));
		lat = static_cast<double *>(malloc(num_voly*sizeof(double)));
		if (lat == nullptr) {
			if (lon != nullptr) free(lon);
			return 1;
		}
		...
		crearFicheroNC(lon_grid, lat_grid, lon, lat, nombre_bati, okada_flag, nombre_fich, &ncid, &time_id, &eta_id, &ux_id,
			&uy_id, num_volx, num_voly, tiempo_tot, CFL, epsilon_h, mf0, vmax, hpos, cvis, dif_at, borde_sup, borde_inf,
			borde_izq, borde_der, es_periodica, numFaults, defTime, LON_C, LAT_C, DEPTH_C, FAULT_L, FAULT_W, STRIKE, DIP,
			RAKE, SLIP, crop_flag, crop_value, H, bati, version);
