...
void disImplicita( double2 Want1,  double2 Want2,
                   double2 *acum1, double2 *acum2, double delta_T,
                  double mf0, double epsilon_h)
{
	...
	double factor, maxheps;
        ...
        u1 = sqrt2 *
             sqrt(Want2.x() * Want2.x() + Want2.y() * Want2.y()) *
             Want1.x() /
             sqrt(Want1.x() * Want1.x() * Want1.x() * Want1.x() +
                        maxheps * maxheps * maxheps * maxheps);

        if (h1 > 0.0)  {
                double c1 = delta_T * mf0 * fabs(u1) /
                            (pow<double>(h1, 4.0 / 3.0) + EPSILON);
                factor = h1/(1.0+c1);
                acum2->x() = factor * u1x;
                acum2->y() = factor * u1y;
        }
...
void filtroEstado(double2 *acum1, double2 *acum2, double vmax,
...
void inicializarEta1MaximaNivel0GPU( double2 *d_datosVolumenes_1,
...
void actualizarEta1MaximaNivel0GPU( double2 *d_datosVolumenes_1,
                                   double *d_eta1_maxima, int num_volx,
...
void obtenerDeltaTVolumenesGPU( double2 *d_datosVolumenesNivel0_3,
                                double2 *d_acumulador_1,
    ...
    nd_item<2> item)
{
	...
	int pos, pos_x_hebra, pos_y_hebra;
		...
		W1 = d_datosVolumenes_1[pos];
                ...
                acum1.x() *= (acum1.x() > 0.0);
                // Start sponge layer
                h0_sponge = max((double)(W1.y() + Hmin + sea_level), 0.0);
                if (tam_spongeIzq > 0) {
...
void obtenerEstadoYDeltaTVolumenesNivel0GPU(
    double2 *d_datosVolumenes_1, sycl::double2 *d_datosVolumenes_2,
    double2 *d_datosVolumenes_3, double *d_anchoVolumenes,
    double *d_altoVolumenes, double2 *d_acumulador_1,
    double2 *d_acumulador_2, double *d_deltaTVolumenes, int num_volx,
	...
	int pos, pos_x_hebra, pos_y_hebra;
		...
		if (tam_spongeIzq > 0) {
			if (pos_x_hebra < tam_spongeIzq) {
				dx = d_anchoVolumenes[pos_y_hebra];
				dist = (tam_spongeIzq-pos_x_hebra-0.5)*dx;
                                spongeLayerPaso2(&(acum2.x()), dist,
                                                 dx * tam_spongeIzq, h0_sponge,
                                                 v0_sponge);
                        }
		}
		if (tam_spongeDer > 0) {
		...
		if (tam_spongeSup > 0) {
			if (pos_y_hebra < tam_spongeSup) {
				dy = d_altoVolumenes[pos_y_hebra];
				dist = (tam_spongeSup-pos_y_hebra-0.5)*dy;
                                spongeLayerPaso2(&(acum2.y()), dist,
                                                 dy * tam_spongeSup, h0_sponge,
                                                 v0_sponge);
                        }
		}
		if (tam_spongeInf > 0) {
...
void escribirVolumenesGuardadoGPU( double2 *d_datosVolumenes_1,
                                   double2 *d_datosVolumenes_2,
                                   double2 *d_datosVolumenesGuardado_1,
                                   double2 *d_datosVolumenesGuardado_2,
                                   ...
                                   nd_item<1> item)
{
	...
	double maxheps, factor;
        ...
        pos_hebra = item.get_group(0) * NUM_HEBRAS_PUNTOS +
                    item.get_local_id(0);
