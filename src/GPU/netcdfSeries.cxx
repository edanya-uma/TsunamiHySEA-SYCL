...
namespace netcdfSeries
{
	...
	int initTimeSeriesNC(char *nombre_bati, char *nombre_fich, int num_points, double *lonPuntos, double *latPuntos, double tiempo_tot,
			double CFL, double epsilon_h, double mf0, double vmax, double hpos, double cvis, double dif_at, double borde_sup,
			double borde_inf, double borde_izq, double borde_der, bool es_periodica, int numFaults, double *defTime, double *LON_C,
			double *LAT_C, double *DEPTH_C, double *FAULT_L, double *FAULT_W, double *STRIKE, double *DIP, double *RAKE, double *SLIP,
			int okada_flag, int crop_flag, double crop_value, double H, char *version)
	{
		...
		iret = nc_put_att_text(ncid_ts, NC_GLOBAL, "problem_name", strlen(nombre_bati), nombre_bati);
		check_err(iret);
		...
		if (es_periodica) {
			...
			iret = nc_put_att_text(ncid_ts, NC_GLOBAL, "left_border", 8, cadena);
			check_err(iret);
			iret = nc_put_att_text(ncid_ts, NC_GLOBAL, "right_border", 8, cadena);
			check_err(iret);
		}
		else {
		...
		if (okada_flag == SEA_SURFACE_FROM_FILE) {
			...
			for (i=0; i<numFaults; i++) {
				...
				iret = nc_put_att_text(ncid_ts, NC_GLOBAL, nombre_falla, strlen(cadena), cadena);
				check_err(iret);
			}
